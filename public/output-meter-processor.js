...
class OutputMeterProcessor extends AudioWorkletProcessor {
  ...
  process(inputs) {
    ...
    let accL = this._accL, accR = this._accR, n = this._n
    let pL = this._pL, pR = this._pR
    let prevL = this._prevL, prevR = this._prevR
    let clipL = this._clipL, clipR = this._clipR
    ...
    if (now - this._lastPost > 1 / 30) {
      ...
      this.port.postMessage({ lRMS, rRMS, lPeak: pL, rPeak: pR, lClip: clipL, rClip: clipR })
    ...
    this._accL = accL; this._accR = accR; this._n = n
    this._pL = pL; this._pR = pR
    this._prevL = prevL; this._prevR = prevR
    this._clipL = clipL; this._clipR = clipR
    return true
  }
}
