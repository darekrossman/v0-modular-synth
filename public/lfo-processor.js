class LFOProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, params) {
    const outBip = outputs[0]?.[0] // −5..+5 V
    const outUni = outputs[1]?.[0] // 0..10 V
    if (!outBip && !outUni) return true

    const N = (outBip || outUni).length
    const sr = sampleRate

    const inFreq = inputs[0]?.[0]
    const inPW = inputs[1]?.[0]
    const inAmp = inputs[2]?.[0]
    const inOff = inputs[3]?.[0]
    const inSync = inputs[4]?.[0]
    ...
    const alpha =
      slew <= 0 ? 1 : 1 - Math.exp(-1 / (sr * (0.002 + 0.25 * slew)))
    ...
    for (let i = 0; i < N; i++) {
      ...
      switch (shape) {
        ...
        case 3:
          ...
          break
        case 4:
          x = this._rndVal + (this._nextVal - this._rndVal) * t
          break
        default:
          x = this._rndVal // stepped
      }
