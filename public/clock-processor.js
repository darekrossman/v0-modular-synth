...
class ClockProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors() {
    return [
      { name: "bpm",  defaultValue: 120, minValue: 0.1, maxValue: 300, automationRate: "k-rate" },
      { name: "div1", defaultValue: 3,   minValue: 0,   maxValue: 8,   automationRate: "k-rate" },
      { name: "div2", defaultValue: 4,   minValue: 0,   maxValue: 8,   automationRate: "k-rate" },
      { name: "div3", defaultValue: 5,   minValue: 0,   maxValue: 8,   automationRate: "k-rate" },
      { name: "div4", defaultValue: 6,   minValue: 0,   maxValue: 8,   automationRate: "k-rate" },
    ]
  }
  ...
  _reset() {
    ...
    this.ppqTick = 0
    this.ppqGate = 0
    this.divGates[0] = this.divGates[1] = this.divGates[2] = this.divGates[3] = 0
  }
  ...
  process(inputs, outputs, parameters) {
    ...
    if (!(bpm > 0) || !isFinite(bpm)) bpm = this.lastBpm
    ...
    const gateWidths = sampleIntervals.map(iv => this._gateWidthForInterval(iv))
