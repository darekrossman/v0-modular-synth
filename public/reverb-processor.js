...
const SIZE_MIN = 0.3, SIZE_MAX = 2.0; // scale for base delays
const DAMP_MIN = 200, DAMP_MAX = 12000; // Hz
const DECAY_MIN = 0.0, DECAY_MAX = 0.98; // feedback gain range
const MIX_MIN = 0.0, MIX_MAX = 1.0;

class ReverbProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors() {
    return [
      { name: "size",      defaultValue: 0.7,  minValue: 0.0,   maxValue: 1.0,   automationRate: "k-rate" },
      { name: "decay",     defaultValue: 0.7,  minValue: 0.0,   maxValue: 1.0,   automationRate: "k-rate" },
      { name: "dampHz",    defaultValue: 6000, minValue: 200,   maxValue: 16000, automationRate: "k-rate" },
      { name: "preDelay",  defaultValue: 0.02, minValue: 0.0,   maxValue: PRE_MAX, automationRate: "k-rate" },
      { name: "mix",       defaultValue: 0.3,  minValue: 0.0,   maxValue: 1.0,   automationRate: "k-rate" },
      // 0 = Room (FDN friendly), 1 = Hall (wider), 2 = Plate (bright)
      { name: "type",      defaultValue: 1,    minValue: 0,     maxValue: 2,     automationRate: "k-rate" },
      // CV depths (0..1). CV signals are expected in [-1..+1].
      { name: "sizeCvAmt",  defaultValue: 0.0, minValue: 0.0,   maxValue: 1.0,   automationRate: "k-rate" },
      { name: "dampCvAmt",  defaultValue: 0.0, minValue: 0.0,   maxValue: 1.0,   automationRate: "k-rate" },
      { name: "decayCvAmt", defaultValue: 0.0, minValue: 0.0,   maxValue: 1.0,   automationRate: "k-rate" },
      { name: "mixCvAmt",   defaultValue: 0.0, minValue: 0.0,   maxValue: 1.0,   automationRate: "k-rate" },
  ...
  process(inputs, outputs, params) {
    ...
    const inSizeCv  = (inputs[1] && inputs[1][0]) ? inputs[1][0] : null;
    const inDampCv  = (inputs[2] && inputs[2][0]) ? inputs[2][0] : null;
    const inDecayCv = (inputs[3] && inputs[3][0]) ? inputs[3][0] : null;
    const inMixCv   = (inputs[4] && inputs[4][0]) ? inputs[4][0] : null;
    ...
    this.dampAlpha = 1 - Math.exp(-2 * Math.PI * Math.max(50, dampHz) / this.sr);
    ...
    const baseDelays = type === 2
      ? [0.010, 0.0127, 0.0153, 0.020] // Plate: shorter, denser
      : type === 1
    ...
    // Pre-delay samples
    let preSamplesTarget = Math.max(0, Math.min(PRE_MAX, preDelay)) * this.sr;
    ...
    for (let i = 0; i < N; i++) {
      ...
      // Effective controls
      const sizeEff = SIZE_MIN + (SIZE_MAX - SIZE_MIN) * Math.max(0, Math.min(1, sizeBase + sCv * 0.5 * sizeCvAmt));
      const decayEff = Math.max(DECAY_MIN, Math.min(DECAY_MAX, decayGainBase + dcV * 0.5 * decayCvAmt));
      const mixEff = Math.max(MIX_MIN, Math.min(MIX_MAX, this.mixZ + mCv * 0.25 * mixCvAmt));
      ...
      this.preW++; if (this.preW >= this.preCap) this.preW = 0;
      ...
      let wetL = 0.5 * ( y0 + 0.8*y1 - 0.6*y2 + 0.3*y3 );
      let wetR = 0.5 * ( 0.3*y0 - 0.6*y1 + 0.8*y2 + y3 );
