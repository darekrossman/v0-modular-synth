...
class LadderFilterProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, params) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const post = Math.max(
        0.25,
        Math.min(2.0, postP.length > 1 ? postP[i] : postP[0]),
      )
      ...
      if (cutoffCv?.[i]) {
        ...
        const cvMultiplier = 2 ** clampedShift
      ...
      let compAlpha = (1 / alpha0) ** (pEff * resC)
