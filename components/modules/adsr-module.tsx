...
export function ADSRModule({ moduleId }: { moduleId: string }) {
  ...
  const [attackN, setAttackN] = useState<number[]>([
    initialParameters?.attackN ?? DEFAULT_ATTACK_N,
  ]);
  const [decayN, setDecayN] = useState<number[]>([
    initialParameters?.decayN ?? DEFAULT_DECAY_N,
  ]);
  const [sustainN, setSustainN] = useState<number[]>([
    initialParameters?.sustainN ?? DEFAULT_SUSTAIN_N,
  ]);
  const [releaseN, setReleaseN] = useState<number[]>([
    initialParameters?.releaseN ?? DEFAULT_RELEASE_N,
  ]);
  ...
  const [longMode, setLongMode] = useState(
    initialParameters?.longMode ?? false,
  );
  const [linearShape, setLinearShape] = useState(
    initialParameters?.linearShape ?? false,
  );
  ...
  useModuleInit(async () => {
    ...
    envOutRef.current = ac.createGain();
    envOutRef.current.gain.value = 1;
    invOutRef.current = ac.createGain();
    invOutRef.current.gain.value = 1;
  ...
  useEffect(() => {
    ...
    if (ac && node)
      node.parameters
        .get('attack')
        ?.setValueAtTime(mapAttack(attackN[0]), ac.currentTime);
  }, [attackN]);
  ...
  useEffect(() => {
    ...
    if (ac && node)
      node.parameters
        .get('decay')
        ?.setValueAtTime(mapDecay(decayN[0]), ac.currentTime);
  }, [decayN]);
  ...
  useEffect(() => {
    ...
    if (ac && node)
      node.parameters
        .get('sustain')
        ?.setValueAtTime(mapSustain(sustainN[0]), ac.currentTime);
  }, [sustainN]);
  ...
  useEffect(() => {
    ...
    if (ac && node)
      node.parameters
        .get('release')
        ?.setValueAtTime(mapRelease(releaseN[0]), ac.currentTime);
  }, [releaseN]);
  ...
  useEffect(() => {
    ...
    if (ac && node)
      node.parameters
        .get('maxv')
        ?.setValueAtTime(mapMaxV(maxVN[0]), ac.currentTime);
  }, [maxVN]);
  ...
  useEffect(() => {
    ...
    if (ac && node)
      node.parameters
        .get('retrig')
        ?.setValueAtTime(retrig ? 1 : 0, ac.currentTime);
  }, [retrig]);
  ...
  useEffect(() => {
    ...
    if (ac && node)
      node.parameters
        .get('long')
        ?.setValueAtTime(longMode ? 1 : 0, ac.currentTime);
  }, [longMode]);
  ...
  useEffect(() => {
    ...
    if (ac && node)
      node.parameters
        .get('shapeLinear')
        ?.setValueAtTime(linearShape ? 1 : 0, ac.currentTime);
  }, [linearShape]);
        ...
        <div className="flex flex-col justify-center items-center flex-1 gap-8">
          <div className="flex gap-4 items-center">
            <ToggleSwitch
              label="Rtrig"
              value={retrig}
              onValueChange={setRetrig}
            />
            <ToggleSwitch
              label="Long"
              value={longMode}
              onValueChange={setLongMode}
            />
            <ToggleSwitch
              label="Lin"
              value={linearShape}
              onValueChange={setLinearShape}
            />
          </div>
