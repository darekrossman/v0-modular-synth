...
function euclidPattern(steps: number, pulses: number, rotate: number): boolean[] {
...
export function EuclidModule({ moduleId }: { moduleId: string }) {
  ...
  const [pulsesNorm, setPulsesNorm] = useState([initialParameters?.pulsesNorm ?? 3 / 8]) // normalized by steps later
  const [rotateNorm, setRotateNorm] = useState([initialParameters?.rotateNorm ?? 0])
  const [gateRatio, setGateRatio] = useState([initialParameters?.gateRatio ?? 0.25])
  ...
  const stepsInt = useMemo(() => Math.max(1, Math.min(16, Math.round((steps[0] ?? 0) * 15 + 1))), [steps])
  const pulsesInt = useMemo(() => Math.max(0, Math.min(stepsInt, Math.round((pulsesNorm[0] ?? 0) * stepsInt))), [pulsesNorm, stepsInt])
  const rotateInt = useMemo(() => Math.max(0, Math.min(Math.max(0, stepsInt - 1), Math.round((rotateNorm[0] ?? 0) * Math.max(0, stepsInt - 1)))), [rotateNorm, stepsInt])
  const pattern = useMemo(() => euclidPattern(stepsInt, pulsesInt, rotateInt), [stepsInt, pulsesInt, rotateInt])
  ...
  useModuleInit(async () => {
    ...
    resetInRef.current = ac.createGain(); resetInRef.current.gain.value = 1
    pulsesInRef.current = ac.createGain(); pulsesInRef.current.gain.value = 1
    rotateInRef.current = ac.createGain(); rotateInRef.current.gain.value = 1
    densityInRef.current = ac.createGain(); densityInRef.current.gain.value = 1
    accentInRef.current = ac.createGain(); accentInRef.current.gain.value = 1
    ...
    const gateOut = ac.createGain(); gateOut.gain.value = 1
    node.connect(gateOut, 0, 0)
    gateOutRef.current = gateOut
    const accOut = ac.createGain(); accOut.gain.value = 1
    ...
    node.port.onmessage = (e) => {
      ...
      if (type === 'step') setCurrentStep((prev) => (value !== prev ? value : prev))
    }
  }, moduleId)
  ...
  const handleStepsChange = useCallback((v: number[]) => {
    const raw = v[0] ?? 0
    setSteps([raw])
    const ac = audioContextRef.current, node = nodeRef.current
    const s = Math.max(1, Math.min(16, Math.round(raw * 15 + 1)))
    if (ac && node) node.parameters.get('steps')?.setValueAtTime(s, ac.currentTime)
  }, [])
  ...
  const handlePulsesChange = useCallback((v: number[]) => {
    ...
    setPulsesNorm([raw])
    const ac = audioContextRef.current, node = nodeRef.current
    if (ac && node) node.parameters.get('pulsesNorm')?.setValueAtTime(raw, ac.currentTime)
  }, [])
  ...
  const handleRotateChange = useCallback((v: number[]) => {
    ...
    setRotateNorm([raw])
    const ac = audioContextRef.current, node = nodeRef.current
    if (ac && node) node.parameters.get('rotateNorm')?.setValueAtTime(raw, ac.currentTime)
  }, [])
  ...
  const handleGateRatioChange = useCallback((v: number[]) => {
    ...
    setGateRatio([raw])
    const ac = audioContextRef.current, node = nodeRef.current
    if (ac && node) node.parameters.get('gateRatio')?.setValueAtTime(raw, ac.currentTime)
  }, [])
  ...
  const handleDensityChange = useCallback((v: number[]) => {
    ...
    setDensity([raw])
    const ac = audioContextRef.current, node = nodeRef.current
    if (ac && node) node.parameters.get('density')?.setValueAtTime(raw, ac.currentTime)
  }, [])
  ...
  const handleAccentChange = useCallback((v: number[]) => {
    ...
    setAccent([raw])
    const ac = audioContextRef.current, node = nodeRef.current
    if (ac && node) node.parameters.get('accent')?.setValueAtTime(raw, ac.currentTime)
  }, [])

  const handleClockDividerChange = useCallback((v: number[]) => {
    const divValues = [1, 2, 4, 8, 16, 32, 64]
    const idx = Math.max(0, Math.min(divValues.length - 1, Math.round((v[0] || 0) * (divValues.length - 1))))
  ...
  const stepTickLabels = useMemo(() => Array.from({ length: 16 }, (_, i) => `${i + 1}`), [])
  const pulsesTickLabels = useMemo(() => Array.from({ length: stepsInt + 1 }, (_, i) => `${i}`), [stepsInt])
  const rotateTickLabels = useMemo(() => Array.from({ length: Math.max(1, stepsInt) }, (_, i) => `${i}`), [stepsInt])
